...
typedef struct
{
	...
	PageId   maxPageId;
	char padding[PageFile::PAGE_SIZE - 2*sizeof(PageId) + sizeof(int)];
} IndexMeta;
...
RC locate_rec(int searchKey, PageId currentPid, IndexCursor& cursor, int d);
  ...
  PageId locate_for_insert(int searchKey);

void printTree(PageId currentPid, int d);

int insert_rec(int key, const RecordId& rid, int d, PageId currentPid, SplitInfo& si);
...
private:
  ...
  int      treeHeight; /// the height of the tree
  PageId   maxPageId; /// the PageId of the most recenlty created node.
