...
{
	memset(this, 0x0, PageFile::PAGE_SIZE);
	numEntries = 0;
	nextLeaf = -1;
}
...
{ 
	//int nodeSize = maxEntries + 2*sizeof(int) + sizeof(PageId);
	char buffer[PageFile::PAGE_SIZE];
	int rc;
...
RC BTLeafNode::insertAndSplit(int key, const RecordId& rid, 
                              BTLeafNode& sibling, PageId siblingPid, int& siblingKey)
{ 
...
void BTLeafNode::print()
{
	printf("[");
	for (int i=0; i<numEntries; i++)
		printf("(%i, %i)| %i ", entries[i].rid.pid, entries[i].rid.sid, entries[i].key);
	printf("|(%i)]", nextLeaf);
}
...
{ 
	...
	sibling.firstPid = overflow[splitPoint].pid; // Set the new node's firstPid to the pointer of the middle node
...
void BTNonLeafNode::print()
{
	...
	printf("[(%i)", firstPid);
	for (int i=0; i<numEntries; i++)
		printf("|%i|(%i)", entries[i].key, entries[i].pid);
	printf("]");
}

PageId BTNonLeafNode::getPidAtEntry(int eid)
{
	if (eid == 0)
		return firstPid;
	else if (eid <= numEntries)
		return entries[eid-1].pid;
	else
		return -1001;
