...
RC BTreeIndex::open(const string& indexname, char mode)
{
	...
	{
	//	fprintf(stderr, "Error: Unable to read the index from disk.  %i\n", rc);
		return rc;
	}
		
	// If the file is not empty, read it's metadata into memory
	...
	if (pf.endPid() != 0)
	{
		...
		if ((rc = pf.read(metaPid, buffer)) < 0)
			return rc;
		memcpy(&im, buffer, PageFile::PAGE_SIZE);
		
		rootPid = im.rootPid;
		treeHeight = im.treeHeight;
		maxPageId = im.maxPageId;
		//printf("rootPid = %i, treeHeight = %i, maxPageId = %i\n", rootPid, treeHeight, maxPageId);

	}

	//printf("About to return from BTreeIndex.open\n");
	return 0;
}
...
RC BTreeIndex::close()
{
	...
	IndexMeta im;
	im.rootPid = rootPid;
	im.treeHeight = treeHeight;
	im.maxPageId = maxPageId;
	memset(im.padding, 0x0, PageFile::PAGE_SIZE - 2*sizeof(PageId) + sizeof(int));
	
	char buffer[PageFile::PAGE_SIZE];
	memcpy(buffer, &im, PageFile::PAGE_SIZE);
	
	if ((rc = pf.write(metaPid, buffer)) < 0)
	{
		fprintf(stderr, "Error: Unable to write the index to disk.  %i\n", rc);
		return rc;
	}
	...
	if ((rc = pf.close()) < 0)
	{
		fprintf(stderr, "Error: unable to close the index.  %i\n", rc);
		return rc;
    }
