...
//  PageFile pf;         /// the PageFile used to store the actual b+tree in disk
  ...
  int      error;      /// error codes used when functions do not work.   
  /// Note that the content of the above two variables will be gone when
  /// this class is destructed. Make sure to store the values of the two 
  /// variables in disk, so that they can be reconstructed when the index
  /// is opened again later.
  
  // Inline member functions used for debugging to set the variables. 
  void setPf(PageFile setpf) {pf = setpf;}

  PageFile getPf() {return pf;}

  void setRoot(PageId setroot) {rootPid = setroot;}

  PageId getRoot() {return rootPid;}

  void setHeight(int setheight) {treeHeight = setheight;}
