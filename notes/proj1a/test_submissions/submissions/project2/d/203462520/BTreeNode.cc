...
void BTNode::print()
{
	...
	DEBUG fprintf(stderr, "Node: %p (pid: %d)\n", nodepage, mypid);
	for (i = 0; i < numnodes; i++) {
		DEBUG fprintf(stderr, "% 5s  % 5s  ", "rid", "key");
	}
	...
	for (i = 0; i < numnodes; i++) {
		...
		sid = nent->rid.sid;
		key = nent->key;
		DEBUG fprintf(stderr, "% 5d  % 5d  ", pid, key);
	}
	DEBUG fprintf(stderr, "\n");
	for (i = 0; i < numnodes; i++) {
		nent = &nodepage[i];
...
{
	//fprintf(stderr, "In locate with searchKey %d...\n", searchKey);
	int keycount = getKeyCount(); // get num keys in node
	int i;
	if (keycount == 0) {
					eid = 0;
					DEBUG fprintf(stderr, "\tLocated eid %d\n", eid);
					return 0; 
	}
	for (i = 0; i <= keycount; i++) { // for each key
					...
					if (entry->key >= searchKey) { // compare entry key
									eid = i; // found key
									DEBUG fprintf(stderr, "* Located eid %d for key %d\n", eid, searchKey);
									return 0; 
					}
	}

	DEBUG fprintf(stderr, "Did not locate an entry!\n");
...
RC BTLeafNode::insert(int key, const RecordId& rid)
{
				...
				keycount = getKeyCount();
				int eid;
				DEBUG fprintf(stderr, "In LN::insert with key %d.\n", key);
				...
				if (eid < keycount) { // if eid is going to overwrite
								...
								DEBUG fprintf(stderr, "Shifting entries...\n");
				...
				DEBUG fprintf(stderr, "Insertion complete:\n");
				print();
...
{ 
				...
				DEBUG fprintf(stderr, "Inserting and Splitting (leaf)\n");
				DEBUG fprintf(stderr, "Original:\n");
				print();
				...
				DEBUG fprintf(stderr, "Splitting entries...\n");
				...
				DEBUG fprintf(stderr, "Original table:\n");
				print();
				DEBUG fprintf(stderr, "New table:\n");
				sibling.print();
...
{ 
				...
				RecordId rid;
				int key;
				if ((rc = readEntry(lastnode, key, rid)) < 0) {
								return rc; // error
				}
...
RC BTNonLeafNode::insert(int key, PageId pid)
{
		...
		DEBUG fprintf(stderr, "Inserting key %d, pid %d into NL node %d...\n", key, pid, mypid);
		print();

		if ((rc = locate(key, eid)) < 0) {
						if (rc == RC_NO_SUCH_RECORD) {
										DEBUG fprintf(stderr, "No match, eid set to %d\n", keycount);
										eid = keycount;
						} else {
							return rc; // error
						}
		...
		if (eid < keycount) {
			/* this is a non-leaf node -- we need to keep the 0th
			 * pointer pointing to the same child
			 */
			// shift array items over
			DEBUG fprintf(stderr, "Inserting new early key to NL node\n");
			int newkey;
			newkey = nodepage[eid].key;
			nodepage[eid].key = key;
			print();
			DEBUG fprintf (stderr, "Shifting %dst onwards...\n", eid);
			nodeEntry nodetemp[numnodes];
			memcpy(&nodetemp, &nodepage[eid + 1], (keycount - (eid + 1))*sizeof(nodeEntry));
			memcpy(&nodepage[eid + 2], &nodetemp, (keycount - (eid + 1))*sizeof(nodeEntry));
			nodepage[eid + 1].key = newkey;
			nodepage[eid + 1].rid.pid = pid;
			nodepage[eid + 1].rid.sid = 0;

			print();

		/* } else if (eid < keycount) {
			// shift array items over one (except for last)
			DEBUG fprintf(stderr, "Shifting items...\n");
			print();
			nodeEntry nodetemp[numnodes];
			memcpy(&nodetemp[0], &nodepage[eid], (keycount - eid )*sizeof(nodeEntry));
			memcpy(&nodepage[eid + 1], &nodetemp[0], (keycount - eid)*sizeof(nodeEntry));
...
{ 
				...
				DEBUG fprintf(stderr, "Locating pointer pid for key %d\n", searchKey);
				print();
				...
				DEBUG fprintf(stderr, "Found pid %d for searchkey %d\n", pid, searchKey);
				return 0; 
}
