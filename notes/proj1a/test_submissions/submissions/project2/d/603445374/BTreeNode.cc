...
RC BTLeafNode::insert(int key, const RecordId& rid) {
	...
	if (getKeyCount() >= BTLeafNode::MAX_PAIRS) {	//if node is full -> insertAndSplit
		...
		int pos;
		if (rc = locate(key, pos) < 0) return rc; //find location to insert
		...
		//if insertion point is in the middle or beginning
			...
			for (int i=getKeyCount()-1; i >= pos; i--) {	//traverse from largest eid down to pos 
...
void BTLeafNode::printNode() {
	cout << "Contents of leafnode" << endl;
	int tempKey = -1;
...
RC BTNonLeafNode::insert(int key, PageId pid) { 
	...
	if (getKeyCount() >= BTNonLeafNode::MAX_PAIRS) {	//if node is full -> insertAndSplit
		...
		//if insertion point is at the middle or beginning, shifting needed
			...
			for (int i=getKeyCount()-1; i >= pos; i--) {	//traverse from furthest down
...
RC BTNonLeafNode::locateChildPtr(int searchKey, PageId& pid) {
	...
	if ( amt == 1 ) {
		memcpy(&comp1, &(buffer[8]), sizeof(int));	//only key to compare with
		if (searchKey < comp1) {	//if searchKey is smaller than only key
			memcpy(&pid, &(buffer[4]), sizeof(int));
			return 0;
		}
		else {
			memcpy(&pid, &(buffer[12]), sizeof(int));
			return 0;
		}
	}
	//will only go for amt=2+
	for (int i=0; i < (amt-1); i++) {
		...
		if ( (i+1) == (amt-1) && searchKey > comp2 ) {	//larger than last key
			memcpy(&pid, &(buffer[8+(i+1)*8+4]), sizeof(int)); //use pid after comp2
			return 0;
		}
	}
...
void BTNonLeafNode::printNode() {
	cout << "Contents of non-leaf node" << endl;
	int tempKey = -1;
	cout << "keycount: " << getKeyCount() << endl;
	for (int i=0; i<getKeyCount(); i++) {
