...
class BTreeIndex {
 public:
  ...
  RC locate(int searchKey, IndexCursor& cursor);
	...
	RC locateLeaf(BTNonLeafNode& nonLeafNode, int level, int searchKey, IndexCursor& cursor);
  ...
  RC readForward(IndexCursor& cursor, int& key, RecordId& rid);
	...
	param rid[IN]
	param split[OUT]
	param siblingKey[OUT]: value is only availabe when split is true
	param pid[OUT]: value is only availabe when split is true
	*/
	RC insertLeafNode(BTLeafNode& leafNode, int leafNodePid, int key, const RecordId& rid, bool& split, int& siblingKey, PageId& siblingPageId);
	...
	param pid[OUT]: value is only availabe when split is true
	*/
	RC findLeafNode(BTNonLeafNode& nonLeafNode, int nonLeafNodePid, int key, const RecordId& rid, bool& split, int& siblingKey, PageId& siblingPageId,
									int level);
	
	//RC insertNonLeafNode(BTNonLeafNode* nonLeafNode, );
 ...
 private:
  ...
  int tmp[PageFile::PAGE_SIZE];
  /// Note that the content of the above two variables will be gone when
  /// this class is destructed. Make sure to store the values of the two 
  /// variables in disk, so that they can be reconstructed when the index
  /// is opened again later.

	int leafNodeIndex[12000];	//store the index of leaf nodes...for debugging
	int leafNodeIndexCount;
	bool isLeaf(int pid);
