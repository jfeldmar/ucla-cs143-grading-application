...
typedef struct TreeList {
		BTLeafNode	*lnode;
		BTNonLeafNode	*nnode;
		PageId		nodeid;		// flag if LeafNode,NonleafNode,RootNode
		// TreeList	*nextnode;	// pointer for linklisting the treelist
}TreeListType;
const int StartPid=4;	// start pageId for the nodes. Before that is the TreeIndex pages
static const int tNodes=(PageFile::PAGE_SIZE/sizeof(TreeListType))*StartPid;
/**
...
class BTreeIndex {
 public:
  ...
  void print();
  PageId getParent(int key,PageId cpid);
  RC incTreecount();
  
 private:
	
	TreeListType treelist[tNodes];
	double		junk[StartPid];
