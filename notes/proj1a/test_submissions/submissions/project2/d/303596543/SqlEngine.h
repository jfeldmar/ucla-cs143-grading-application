...
class SqlEngine {
 public:
  ...
  static RC selectWithIndex(RecordFile& rf,BTreeIndex& bt,const std::vector<SelCond>& cond,int attr);
  static RC selectAll(RecordFile& rf,const std::vector<SelCond>& cond,int attr);
  static void printTuple(int key,std::string value,int attr,int count);
};
