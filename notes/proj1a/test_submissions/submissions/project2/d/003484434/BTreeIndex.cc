...
RC BTreeIndex::insert_into_parent(PageId pid1, int key, PageId pid2)
{
   if(pid1 == rootPid)
   {
	...
	rootPid = pf.endPid();			// update the pid of the root
	new_root.set_own_pid(rootPid);	// this is the new root
