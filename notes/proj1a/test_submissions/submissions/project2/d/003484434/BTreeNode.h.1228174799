...
class BTLeafNode {
  public:
    ...
    {
    	int initial_size = 0;
	memcpy(buffer+4, &initial_size, 4);
	int node_type = 1;	// leaves are type 1 for nodetype in buffer+0
	memcpy(buffer, &node_type, 4);
    }
/**
    ...
    */
    ...
    // set the parent pid for this node
    RC set_parent_pid(PageId pid);
    
    // set the pid for this node
    RC set_own_pid(PageId pid);

    PageId get_own_pid();

    PageId get_parent_pid();
   /**
    * Insert the (key, rid) pair to the node
...
private:
    char buffer[PageFile::PAGE_SIZE]; 	// for reading into
    const static int MAX_KEYS = 75; 	// MAX_KEYS-1 to take care of insertAndSplit
...
class BTNonLeafNode {
  public:
    ...
    BTNonLeafNode()
    {
    	int initial_size = 0;
	memcpy(buffer+4, &initial_size, 4);
	int node_type = 0;	// non-leaves are type 0 for nodetype in buffer+0
	memcpy(buffer, &node_type, 4);
    }
