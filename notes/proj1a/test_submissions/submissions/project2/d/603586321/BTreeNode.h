...
#define MAX_LEAF_KEYS (PageFile::PAGE_SIZE - sizeof(int) - sizeof(PageId)) / (sizeof(int) + sizeof(RecordId))
#define MAX_NONLEAF_KEYS (PageFile::PAGE_SIZE - sizeof(int) - sizeof(PageId)) / (sizeof(int) + sizeof(PageId))

// Struct that holds the leaf node data
typedef struct {
	int keys[MAX_LEAF_KEYS];
	RecordId rids[MAX_LEAF_KEYS];
	PageId  sibling;
	int key_count;
	char buffer[PageFile::PAGE_SIZE - (MAX_LEAF_KEYS * (sizeof(int) + sizeof(RecordId)) + sizeof(int) + sizeof(PageId))];
} LeafNodeStruct;

// Struct that holds the non-leaf node data
typedef struct {
	int keys[MAX_NONLEAF_KEYS];
	PageId pids[MAX_NONLEAF_KEYS + 1];
	int key_count;
	char buffer[PageFile::PAGE_SIZE - (MAX_NONLEAF_KEYS * (sizeof(int) + sizeof(PageId)) + sizeof(int) + sizeof(PageId))];
} NonLeafNodeStruct;
