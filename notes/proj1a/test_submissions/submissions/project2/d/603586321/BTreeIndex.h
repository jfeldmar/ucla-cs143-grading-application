...
#define MAX_CHARS (PageFile::PAGE_SIZE - sizeof(PageId) - sizeof(int)) / sizeof(char)
...
class BTreeIndex {
 public:
  ...
  RC readForward(IndexCursor& cursor, int& key, RecordId& rid);
	...
	RC path_to_leaf(int searchKey, PageId path[]);

	RC insert_in_parent(PageId path[], int parent_pos, int new_key, PageId new_pid);

	RC create_tree(int key, const RecordId& rid);
