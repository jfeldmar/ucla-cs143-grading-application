...
class BTLeafNode 
{
  public:
	...
	BTLeafNode() {keyCount = 0; sibPid = -1; parPid = -1;};
	// max number of tuples
	static const int MAX_TUPLES = 84;
    ...
    */
	...
	// set parPid
	void setParPid(int pid);
	
	// get parPid
	int getParPid();
	
/*	Storage Locations in pageFile as array of 256 ints
int  0		= leaf
int  1		= keyCount
int  2		= sibPid
ints 3-254	= entry[key, pid, sid]
int	 255	= parent pid
*/
  private:
	// to check if this node is a leaf
	int leaf;  
...
class BTNonLeafNode
{
  public:
	// default constructor
	BTNonLeafNode() {keyCount = 0; firstChildPid = -1; parPid = -1;};
	// max number of tuples
	static const int MAX_TUPLES = 126;
	// point to split tuples in node
	static const int BREAK_POINT = MAX_TUPLES / 2;
    ...
    */
	...
	void setFirstChildPid(int pid);
	
	// get the first child pid
	int getFirstChildPid();
	...
	RC setChildrenParPids(int pid, PageFile& pf);
	
/*	Storage Locations in pageFile as array of 256 ints
entry[0]	= leaf
entry[1]	= keyCount
entry[2]	= firstChildPid
entry[3-254]= entry[key, pid]
entry[255]	= parent pid
*/
  private:
	// leaf indicator
	int leaf;
