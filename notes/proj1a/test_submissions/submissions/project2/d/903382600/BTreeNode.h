...
class BTLeafNode {
  public:
   ...
   /**
    *A page file consists of RECORDID having size 8
    *and an INT key having size 4.
    *The maxsize of a pagefile is 1024 thus n = 1024 - 4 - 4 = 1016
    *             (recordid + key = 12 bytes) = 1016 / 12 = 84
    *    +1 to compensate for sibling pointer = 85
    *The last 4 bytes on the page is used to point at its sibling leaf node
    *First 4 bytes used to store keycount
    */
   static const int MAX_LEAF_POINTER = 85;
    ...
    */
    ...
    private:
    char content[PageFile::PAGE_SIZE];
    int keycount;
}; 
...
class BTNonLeafNode {
  public:
   /**
    *A page file consists of nodes and keys all of which are int values thus having
    *a size of 4 each.
    *The maxsize of a pagefile is 1024 thus n = ((((1024 - 4) / 4) -1) / 2) + 1 = 128
    *Note that the first 4 bytes on a page is used to store the number of keys
    *In the page.
    */
   static const int MAX_NONLEAF_POINTER = 128;
