...
class BTreeIndex {
 public:
  ...
  //PageID getRoot
  void setPageFile(PageFile& newPf)
  {
	  pf = newPf;
  }
  void setRoot(PageId newRoot)
  {
	  rootPid = newRoot;
  }
...
public:
  ...
  RC loadMemberVariables();	//get rootPid, treeHeight from disk
  RC setMemberVariables();	//save rootPid, treeHeight to disk
 ...
 */
  PageId insertHelper(PageId pid, int depth, int& key, const RecordId& rid);

  void printNode(int n, bool isLeafNode);
};
