...
RC BTreeIndex::locate(int searchKey, IndexCursor& cursor)
{
	...
	//Special case where searchKey > all keys
...
RC BTreeIndex::readForward(IndexCursor& cursor, int& key, RecordId& rid)
{
	...
	rc = leaf.readEntry(cursor.eid, key, rid);	if (rc != 0) return rc;
...
void BTreeIndex::print(int height, PageId pid)
{
	if (height == treeHeight - 1)	{
		BTLeafNode node;
		...
		if (node.getKeyCount() >= 1)	{
			fprintf(stderr,"Getting first pid: %d\n",node.get_pid(-1));
			print(height+1,node.get_pid(-1));
		}
		for (int i = 0; i < node.getKeyCount(); ++i)	{
			fprintf(stderr,"Getting another pid: %d\n",node.get_pid(i));
			print(height+1,node.get_pid(i));
		}
		node.free();
		return;
	}	
