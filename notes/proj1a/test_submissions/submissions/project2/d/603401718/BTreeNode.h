...
public:
	...
	*/
	void print()
	{	
		fprintf(stderr,"LeafNode with %d keys\n", count) ;
		for (int i = 0; i < count; ++i)	{
			fprintf(stderr,"%d. Key: %d RecordPid: %d RecordSid: %d\n",i,buffer[i].key,buffer[i].rid.pid,buffer[i].rid.sid);
		}
		fprintf(stderr,"Last PageId: %d\n", next_pid) ;
	}
...
public:
	...
	*/
	void print()
	{
		fprintf(stderr,"NonLeafNode with %d keys\nFirst PageId: %d\n", count, first_pid) ;
		for (int i = 0; i < count; ++i)	{
			fprintf(stderr,"%d. Key: %d PageId: %d\n",i,buffer[i].key,buffer[i].pid);
		}
	}
