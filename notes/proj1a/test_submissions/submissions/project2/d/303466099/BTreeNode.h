...
const int MAX_NUM_LEAF_PTRS = 86;
const int MAX_NUM_NONLEAF_PTRS = 128;

struct Leaf{
	int      keyCount;
	long     keyArr[MAX_NUM_LEAF_PTRS - 1];
	RecordId ridArr[MAX_NUM_LEAF_PTRS - 1];
	PageId   nextLeafNode;
};

struct NonLeaf{
	int      keyCount;
	int      keyArr[MAX_NUM_NONLEAF_PTRS - 1];
	PageId   pidArr[MAX_NUM_NONLEAF_PTRS];
};
...
class BTLeafNode {
  public:
   ...
   /**
    * Retrieve the pointer to a Leaf structure.
    * @return Leaf pointer.
    */
    Leaf * getLeafPtr();

   /**
    * Debugging purposes. Prints out contents of leaf node.
...
class BTNonLeafNode {
  public:
   ...
   /**
    * Retrieve the pointer to a Non-Leaf structure.
    * @return Non-Leaf pointer.
    */
    NonLeaf * getNonLeafPtr();

   /**
    * Helper function. Locates a key within a Non-Leaf node.
    * @param searchKey[IN] the key to search for
    * @return 0 if successful.
    */
    RC locate(int searchKey, int &eid); 
