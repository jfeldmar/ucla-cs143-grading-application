...
RC BTreeIndex::insert(int key, const RecordId& rid)
{
	...
	{	
		...
		while(more == 0)
		{
			...
			if((track > treeHeight) && (more == 0))
			{
				treeHeight++;
				ntree.initializeRoot(curpid, newkey, end);
				rootPid = pf.endPid();
				ntree.write(rootPid, pf);
				more = 1;
				break;
			}
			else
...
RC BTreeIndex::locate(int searchKey, IndexCursor& cursor)
{
	...
	int error = ltree.locate(searchKey, entryid);	
	if((error == -1012))
	{
		cursor.pid = nextpid;
		cursor.eid = 1;
...
{
	BTLeafNode btree;
	btree.read(cursor.pid, pf);
	int last = (btree.buffer[0]*3)+1;
	btree.readEntry(cursor.eid, key, rid);
	if(btree.buffer[0] == cursor.eid)
	{
		cursor.pid = btree.buffer[last];
		cursor.eid = 1;
		if(cursor.pid == -1)
		{return(-1013);}
	}
