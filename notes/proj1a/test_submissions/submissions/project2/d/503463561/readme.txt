...
================================================================================
PROJECT 2 PART C README
================================================================================

For the third part of our project, we added a few extra functions to the B+ tree
index interface: print() to print out the internal B+ tree for debugging, drop()
(unused) to drop all the contents of the current index file, and getNumEntries()
to get the number of entries registered with the current index file. print() was
especially useful for debugging, as it performed a pre-order traversal of the
tree and allowed easy observation of node splitting.

The recursion in the B+ tree insertion and location algorithm is performed not
using recursive function calls but rather through a while loop augmented with a
stack of node locations. This reduced overhead significantly when searching
through the tree.

================================================================================
PROJECT 2 PART D README
================================================================================

The most challenging part of this project was simply figuring out how to use the
B+ tree in the most efficient way to limit the amount of reads needed to perform
a SELECT statement. My SqlEngine::select() function first tries to open a B+
tree index for searches. Then it tries to determine the range of possible values
that need to be traversed by the B+ tree; if the range is narrowed down to the
point that the interval is empty, we can return early without ever having to
physically access the table. If we do, however, we simply use our B+ tree
index to access only those values that fall within the interval, by locating the
first value and reading forward until the key is greater than the last. We can
also exploit the B+ tree to speed up SELECT COUNT(*) statements; if no
conditions exist then we can simply return the number of entries in the B+ tree
as a cached value for the number of tuples in our table.
