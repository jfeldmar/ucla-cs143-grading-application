...
{ 
	//DELETE ME
	/* if(key == 3754)
		cout<<"Inserted Simpatico at leafNode insert level\n"; */
	...
	/* if(key == 3754)
		cout<<"INSERTED! with pid:"<<rid.pid <<" and sid: "<<rid.sid<<endl; */
                              ...
                              BTLeafNode& sibling, int& siblingKey)
{
	...
	if(insertPos <= 42)
	{
		...
		int initial = -1;
		for (int i=N_LEAF/2/*start at pos 42*/; i<N_LEAF; i++)
			memcpy(leafArray + (i * 12), &initial, sizeof(int));
...
{ 
	...
	int insertPos;
	insertPos=0;
	while(key > getKey(insertPos) && insertPos != usedEntries-1 && insertPos < getKeyCount())
		insertPos++;
	
	//fix:
	PageId tpid = getPid(insertPos);
	if(key == getKey(insertPos))
	{
		setPid(insertPos + 1, pid);
		setPid(insertPos, tpid);
		return 0;
	}

	int i;
	if (key <= getKey(insertPos) && insertPos != usedEntries - 1){
...
{ 
	...
	int insertPos;
	insertPos=0;
	while(key > getKey(insertPos) && insertPos != N_NONLEAF/2 && insertPos < getKeyCount())
		insertPos++;
	
	//If insertpos == N_NONLEAF/2 (or 63), insert the new key in the sibling, else in the  caller.
	if (insertPos <= N_NONLEAF/2) {
		memcpy(sibling.nonLeafArray, nonLeafArray + (((N_NONLEAF)/2) * 8),((N_NONLEAF)/2 + 1) * 8);
		...
		for (i=(N_NONLEAF)/2; i<N_NONLEAF; i++)
			memcpy(nonLeafArray + (i * 8),&initial,sizeof(int));		
		insert(key, pid);
		midKey = getKey(getKeyCount() - 1);
		this->setPid(getKeyCount() - 1, -1);
	}
	else {
		memcpy(sibling.nonLeafArray, nonLeafArray + ((N_NONLEAF/2 + 1) * 8),((N_NONLEAF)/2) * 8);
		//Reset pid for empty entries in caller node (from N_NONLEAF onwards)
		int i;
		int initial = -1;
		for (i=N_NONLEAF/2 + 1; i<N_NONLEAF; i++)
			memcpy(nonLeafArray + (i * 8),&initial,sizeof(int));		
...
{ 
	...
	int location;
	location=0;
	while(searchKey > getKey(location) && location < getKeyCount() - 2)
		location++;
		
	/*if(location == getKeyCount())
	{
