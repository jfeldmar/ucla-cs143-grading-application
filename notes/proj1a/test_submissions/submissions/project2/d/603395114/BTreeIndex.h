...
class BTreeIndex {
 public:
   ...
   */
  RC printIndex();
  RC printIndexNonLeaf(BTNonLeafNode& n, int height);
  RC printIndexLeaf(BTLeafNode& leaf);
  void getStats(btstats& stats);
 ...
 private:
  ...
  struct index_data {
    ...
    int    min;
    int    num_entries;
    char   padding[PageFile::PAGE_SIZE - 40];
  } d;
  BTNonLeafNode root;
  PageId cached_pid;
  BTLeafNode cached_node;

  RC insertRoot(int& skey, PageId& spid);
  RC insertNonLeaf(BTNonLeafNode& n, const PageId wpid, int& skey, PageId& spid);
  RC insertHelper(BTNonLeafNode& n, int key, const RecordId& rid, int height, int& skey, PageId& spid);
  RC locateHelper(BTNonLeafNode& n, int key, IndexCursor& cursor, int height);
};
