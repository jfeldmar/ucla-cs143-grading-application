...
Our implementation will choose to use the index when there is an .idx file for that table query and when there is a key constraint. However, if the query is something like select * from movie with no constraints, or if no .idx file is found, it will choose to use the original select implementation.
