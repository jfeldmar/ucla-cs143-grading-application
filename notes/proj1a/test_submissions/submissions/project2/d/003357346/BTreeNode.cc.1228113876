...
int BTLeafNode::getHeight() {
	int height;
	memcpy(&height, m_page + TREE_HEIGHT_INDEX, sizeof(int));
	return height;		
}

void BTLeafNode::setHeight(int height) {
	memcpy(m_page + TREE_HEIGHT_INDEX, &height, sizeof(int));
}
...
{ 
	...
	setKeyCount(getKeyCount() - numKeys);
...
void BTNonLeafNode::setHeight(int height) {
	memcpy(m_page + TREE_HEIGHT_INDEX, &height, sizeof(int));
}
