...
#define ROOT_PID	0
#define TREE_HEIGHT	ROOT_PID + sizeof(int)
#define NUM_PIDS_USED	TREE_HEIGHT + sizeof(int)
...
class BTreeIndex {
 public:
  ...
  RC readForward(IndexCursor& cursor, int& key, RecordId& rid);
	...
	RC insert_helper(int key, const RecordId rid, PageId pid, int currHeight, int& keyToParent,
					PageId& pidToParent, bool& overflow);			//attempt to insert into PageId pid

	void printTreeHelper(BTNonLeafNode nl);		
	void printTree();	
	void printMeta();
	void findLeftMost(IndexCursor& cursor);
	void flm_helper(IndexCursor& cursor, int height, PageId pid);

	RC locate_helper(int searchKey, PageId pid, IndexCursor& cursor, int currHeight);
	PageId getRoot();
	PageFile* getPf();
