...
class SqlEngine {
 public:
  ...
  // scan the entire table and print out matching tuples
  static RC scanTable(RecordFile &rf, int attr, const std::string& table, const std::vector<SelCond>& conds);
  
  // scan a range of tuples from [low to high)
  static RC scanRange(BTreeIndex &indexFile, RecordFile &rf, int attr, const std::string& table, const std::vector<SelCond>& conds, IndexCursor& low, IndexCursor& high);

  // locate a single record and check it against the conditions
  static RC locateSingle(RecordFile &rf, int attr, const std::string& table, const std::vector<SelCond>& conds, RecordId& rid);
