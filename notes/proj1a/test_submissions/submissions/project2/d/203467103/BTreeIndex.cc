...
RC BTreeIndex::open(const string& indexname, char mode)
{
	...
	if (!rc) {
		if (pf.endPid() == 0) { // if the zero-th entry hasn't been written yet
    ...
    return RC_FILE_READ_FAILED; // means that the file had to be created;
}
