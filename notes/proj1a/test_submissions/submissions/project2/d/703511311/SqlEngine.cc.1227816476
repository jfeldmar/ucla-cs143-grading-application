...
{
	int equal = -1; 				// check equal attribute
	int lower = -1; 				// lower bound
	int upper = -1; 				// upper bound
		...
		if (cond[i].attr == 1) // numeric conditions
		{
			switch (cond[i].comp)
			{
				...
				case SelCond::GT:
					...
					break;
				case SelCond::LT:
					if (atoi(cond[i].value) < upper || upper == -1)
	...
	if (equal != -1)
	{
		if (upper != -1 && equal > upper) goto exit_idx;
		if (lower != -1 && equal < lower) goto exit_idx;
	...
	do
	{
		...
		if (upper != -1 && key > upper) goto attr_count;
