...
RC BTLeafNode::insert(int key, const RecordId& rid)
{
	  ...
	  for (; i < node.size; i++)
	  {
		  if (key < node.data[i].key || node.data[i].key == INT_MIN)
		  {
			  break;
		  }
	  }

	  if (node.data[i].key == INT_MIN)
	  {
		  node.data[i].key = key;
		  node.data[i].rid = rid;
	  }
                              ...
                              BTLeafNode& sibling, int& siblingKey)
{
	...
	if (key < node.data[node.size-1].key)
	{
		...
		node.data[node.size-1].key = INT_MIN;
		node.numOfData--;
		insert(key, rid);
	}
	else
...
{
	if (node.data[eid].key == INT_MIN) return RC_INVALID_CURSOR;
...
RC BTNonLeafNode::insertAndSplit(int key, PageId pid, BTNonLeafNode& sibling, int& midKey)
{
	...
	node.data[half].key = INT_MIN;
	node.data[half+1].key = INT_MIN;
