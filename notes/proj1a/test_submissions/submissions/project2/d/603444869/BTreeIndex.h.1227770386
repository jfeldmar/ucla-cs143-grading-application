...
class BTreeIndex {
 public:
  ...
  RC open_helper(const std::string& indexname, char mode);
   ...
   * @return error code. 0 if no error
   */
RC insert_split(int key, const RecordId& rid, PageId curr_pid, int curr_level);
 // RC insert_helper(int key, const RecordId& rid);
  RC insert(int key, const RecordId& rid);
  RC insert_real(int key, const RecordId& rid);
PageId leftmost_change(int searchKey, PageId left_pid, IndexCursor& cursor, int curr_level);
   ...
   * @return error code. 0 if no error.
   */
PageId non_leaf_locate(int searchKey, IndexCursor& cursor, int curr_level, int stop_level);
  RC locate_helper(int searchKey, IndexCursor& cursor, int curr_level);
  RC locate(int searchKey, IndexCursor& cursor);

  RC Debug(PageId curr, int curr_level);
  ...
  RC readForward(IndexCursor& cursor, int& key, RecordId& rid, bool getnext);
