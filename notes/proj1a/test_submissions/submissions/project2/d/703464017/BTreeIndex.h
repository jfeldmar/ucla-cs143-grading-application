...
class BTreeIndex {
 public:
  ...
  //RC getSmallestKey(int& key, IndexCursor& cursor);
  RC getSmallestKey(int& key);

  int getRootPid();
  int getTreeHeight();
  void setRootPid(PageId pid);
  void setTreeHeight(int height);

  RC insert_entry(PageId pid, int key, const RecordId& rid);
  RC insert_entry(PageId pid, int key, const PageId& pid);
  RC print_tree(PageId pid, int level);
  RC print_node(PageId pid, int level);
  RC print_forward(IndexCursor& cursor);
