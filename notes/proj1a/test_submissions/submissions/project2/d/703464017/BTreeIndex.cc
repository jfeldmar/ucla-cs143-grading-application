...
RC BTreeIndex::insert(int key, const RecordId& rid)
{
    ...
    if (cursor.pid < 0) {
        ...
        // Create a new page as leaf to store the new smallest key
        cursor.pid = pf.endPid();
        memcpy(nonLeaf.buffer + sizeof(int), &cursor.pid, SIZE_PID);
        nonLeaf.write(pid, pf);

        // Update the new leaf's last pointer
        memcpy(&pid, nonLeaf.buffer + sizeof(int) + SIZE_NONLEAF_PAIR, SIZE_PID);
        leaf.read(cursor.pid, pf);
        leaf.setNextNodePtr(pid);
        leaf.write(cursor.pid, pf);
    }

    // Concept: L = cursor.pid
...
//            if (cursor.pid < 0)
//                // Reach the NULL pointer, and the nonLeaf is always the last
//                // level of NON leaf node
//                memcpy(&cursor.pid, nonLeaf.buffer + sizeof(int) + SIZE_NONLEAF_PAIR, SIZE_PID);
...
RC BTreeIndex::readForward(IndexCursor& cursor, int& key, RecordId& rid){
    ...
    location = leaf.buffer + sizeof(int) + cursor.eid*SIZE_LEAF_PAIR;

    memcpy(&rid.pid, location, SIZE_PID);
    memcpy(&rid.sid, location + SIZE_PID, sizeof(int));
    memcpy(&key, location + SIZE_PID + sizeof(int), sizeof(int));


    if (cursor.eid < leaf.getKeyCount() - 1) {
        // Increse the eid but the pid will stay the same
        cursor.eid++;
    }
    else {
        cursor.eid = 0;
        // If the eid is the last record, read the next leaf, if any
        cursor.pid = leaf.getNextNodePtr();
            // Set the eid as the first record of the next leaf
 ...
 * @param key[OUT] the smallest key
 * @param cursor[OUT] the cursor pointing to the smallest key
 * @return error code. 0 if no error
 */
//RC BTreeIndex::getSmallestKey(int& key, IndexCursor& cursor) {
RC BTreeIndex::getSmallestKey(int& key) {
    RC rc;
    PageId pid;
    ...
    memcpy(&pid, root.buffer + sizeof(int), SIZE_PID);

    while(level-- > 2) {
    if ((rc = nonLeaf.read(pid, pf)) < 0) return rc;
        memcpy(&pid, nonLeaf.buffer + sizeof(int), SIZE_PID);
    }
...
//    cursor.eid = 0;
    ...
    memcpy(&key, leaf.buffer + sizeof(int) + SIZE_RID, sizeof(int));
...
RC BTreeIndex::print_forward(IndexCursor& cursor) {
    
    int key;
    RecordId rid;

    fprintf(stdout, "[%d,%d] - ", cursor.pid, cursor.eid);
