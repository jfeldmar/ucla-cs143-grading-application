...
RC BTreeIndex::locate(int searchKey, IndexCursor& cursor)
{
	...
	if (treeHeight < 0)
	{
		//std::cout << "TREE HEIGHT ERROR" << std::endl;
		return ERROR;
	}
	...
	{
		//std::cout << "tempLeaf not read" << std::endl;
		return ERROR;
	}
	tempLeaf.locate(searchKey, cursor.eid);
	cursor.pid = currentPid;
	//std::cout << cursor.pid << " " << cursor.eid;
	return 0;
}
 ...
 * @return error code. 0 if no error
 */
/* #FIX ME# */ RC BTreeIndex::readForward(IndexCursor& cursor, int& key, RecordId& rid)
{
	//std::cout << cursor.pid << " " << cursor.eid << std::endl;
	BTLeafNode tempNode;
	if (tempNode.read(cursor.pid, pf))
	{
		//std::cout << "tempNode not read" << std::endl;
		return ERROR;
	}
