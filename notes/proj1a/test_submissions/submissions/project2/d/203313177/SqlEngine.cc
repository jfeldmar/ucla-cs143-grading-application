...
RC SqlEngine::select(int attr, const string& table, const vector<SelCond>& cond)
{
  ...
  if ((rc = rf.open(table + ".tbl", 'r')) < 0) {
	...
	int temp = 0;
	if ((myrc = tableIndex.open(table + ".idx", 'r')) >= 0 && keyCondition)
	{
		for (unsigned i = 0; i < keyCond.size(); i++)
		{
			myKey = atoi(keyCond[i].value);
			switch (keyCond[i].comp)	{
				case SelCond::EQ:
					tableIndex.locate(myKey, myic);
					tableIndex.readForward(myic, myKey, myrid);
					rf.read(myrid, myKey, myVal);
					goto output;
					break;
				case SelCond::GT:
					tableIndex.locate(myKey+1, myic);
					tableIndex.readForward(myic, myKey, myrid);
					rf.read(myrid, myKey, myVal);
					goto output;
					break;
