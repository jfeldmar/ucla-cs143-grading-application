...
#define BTLEAF_N (PageFile::PAGE_SIZE/(sizeof(RecordId)+sizeof(int))) // == 85
#define BTNONLEAF_N (PageFile::PAGE_SIZE/(sizeof(PageId)+sizeof(int))) // == 128


/**
 * For use in BTLeafNode
 *
class Rid_key_pair {
  public:
    RecordId rid;
    int key;
    Rid_key_pair(RecordId r, int k) {
			rid = r;
			key = k;
    };
    bool operator<(const Rid_key_pair &other) const {
...
class BTLeafNode {
  public:
#if (richard_debug==1)
    void printNode() {
      for (map<int, RecordId>::iterator itr = node.begin();itr != node.end();++itr) {
        cout<<"Pointer: "<<itr->first<<";"<<"Pid: "<<(itr->second).pid<<";"<<"Sid: "<<(itr->second).sid<<endl;
      }
    }
#endif
   /**
    ...
    */
    ...
    PageId getPid() { return node_pid; };
  private:
    map<int, RecordId> node;
    map<int, RecordId>::iterator internal_itr;
    int itr_pos;
    PageId node_pid; // pageid to store this node
}; 


/**
 * For use in BTNonLeafNode
 *
class Pid_key_pair {
  public:
    PageId pid;
    int key;
    Pid_key_pair(PageId p, int k) {
	pid = p;
	key = k;
    };
    bool operator<(const Pid_key_pair &other) const {
	return this->key < other.key;
    };
};
*/
