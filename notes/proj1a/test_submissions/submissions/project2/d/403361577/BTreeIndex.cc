...
RC BTreeIndex::readForward(IndexCursor& cursor, int& key, RecordId& rid)
{
    ...
    // update cursor
    if (eid+1 > MAX_KEY_PER_PAGE || eid+1 > leafNode.getKeyCount()) {
...
// gets the min key of the index
int BTreeIndex::minKey()
{
    int min_key = 0;
    ...
    if (rootNode.read(rootPid, pf) != 0)
        return -1;
    // locate the child pointer
    if (rootNode.locateChildPtr(min_key, childPid) != 0) {
        return -1;
        //printf("\nFAILED TO LOCATE FROM ROOT\n");
    }
    // update tree level
    ...
    while (treeLevel < treeHeight) {
        ...
        if (nonLeafNode.read(childPid, pf) != 0)
            return -1;
        // locate the next child pointer
        if (nonLeafNode.locateChildPtr(min_key, childPid) != 0) {
            //printf("\nFAILED TO LOCATE FROM NON-LEAF\n");
            return -1;
        }
        treeLevel++;
    ...
    if (leafNode.read(childPid, pf) != 0)
        return -1;
    // prevent from reading from a blank leaf
    while (leafNode.getKeyCount() == 0) {
        childPid = leafNode.getNextNodePtr();
        // read the leaf node
        if (leafNode.read(childPid, pf) != 0)
            return -1;
    }

    // get the min key of the leaf node
    memcpy(&min_key, leafNode.buffer + sizeof(int), sizeof(int));
