...
const int BTNonLeafNode_N = 80;////120; // could be 128
const int BTLeafNode_N = 80;//////80;// could be 85
...
class BTLeafNode {
  public:
    ...
    */
	...
	BTLeafNode* BTLeafNodeLocator(PageId pid);

	void		setParent(PageId pid);
	PageId		getParentPid();
	PageId		getPid();
	RecordId	ridFinder(int index);
  ...
  private:
	...
	RecordId		nodeRid[BTLeafNode_N];
	int				nodeKey[BTLeafNode_N];
	PageId			nodeLast;
...
class BTNonLeafNode {
  public:
    ...
    */
	...
	BTNonLeafNode* BTNonLeafNodeLocator(PageId pid);
...
private:
	...
	PageId			nodePid[BTNonLeafNode_N+1];
	int				nodeKey[BTNonLeafNode_N];
	int				keyCount;
