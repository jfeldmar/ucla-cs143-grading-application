...
class BTreeIndex {
 public:
  ...
  // helper function
	void fixTree(int key, PageId selfpid, PageId parentpid, int level);
  ...
  // helper function
    IndexCursor findLeaf(int searchKey, PageId root, int height);
  ...
  void printEntireTree(PageId selfpid, int level);
  PageId getRoot();
  RecordId ridFinder(IndexCursor cursor);
