...
bool operator<(const LeafPair& a, const LeafPair& b) {
    return a.key < b.key;
}
...
bool operator<(const NonLeafPair& a, const NonLeafPair& b) {
    return a.key < b.key;
}
	...
	*/
	void print()
	{
		...
		for (int i = 0; i < count; ++i)	{
			std::cout << "Key: " << buffer[i+1].key << " RecordPid: " << buffer[i+1].rid.pid  
				<< " RecordSid: " << buffer[i+1].rid.sid << endl;			
		}
		std::cout << "Last PageId: " << next_pid << endl;
	}
...
class BTNonLeafNode {
  public:
   ...
   /**
	* Helper function setting first_pid
	*/
	void set_first_pid(PageId pid)	{first_pid = pid;}
	...
	*/
	void print()
	{
		std::cout << "NonLeafNode\n";
		std::cout << "First PageId: " << first_pid << endl;
		for (int i = 0; i < count; ++i)	{
			std::cout << "Key: " << buffer[i+2].key << " PageId: " << buffer[i+2].pid << endl;
		}
	}

	//Helper function for retrieving node data
	PageId get_pid(int eid)	{
		if (eid == -1)
			return first_pid;
		else
			return buffer[eid+2].pid;
	}
