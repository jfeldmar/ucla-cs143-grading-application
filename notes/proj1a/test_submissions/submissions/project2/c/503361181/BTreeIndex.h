...
class BTreeIndex {
 public:
  ...
  RC recInsert(int key, const RecordId& rid, int height, PageId& pid, BTLeafNode& leafsibling, BTNonLeafNode& nonleafsibling, int& siblingKey);
  
  RC recFind(int searchKey, IndexCursor& cursor, int height, PageId& pid);
