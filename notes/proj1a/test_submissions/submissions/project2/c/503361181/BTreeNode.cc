...
{ 
	...
	else
	{
		...
		{
				if(nonleaf[i*2+1] == -1 && nonleaf[i*2+2] == -1) break;
		...
		if(c == getKeyCount()-1){ int t = nonleaf[c*2]; nonleaf[c*2] = nonleaf[c*2+2]; nonleaf[c*2+2] = t; }
	}
	return 0; 
...
{ 
	...
	for(;j<N; j++) { sibling.nonleaf[j*2] = -1; sibling.nonleaf[j*2+1] = -1; }
