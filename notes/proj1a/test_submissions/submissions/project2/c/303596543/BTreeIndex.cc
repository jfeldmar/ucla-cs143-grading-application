...
const int StartPid=1;	// start pageId for the nodes. Before that is the TreeIndex pages
/*
...
{
    rootPid = -1;
//	fprintf(stdout, "BTreeI tNodes %d treelist %d bytes array %d bytes\n",tNodes,sizeof(TreeListType),
//		sizeof(treelist));
}
...
RC BTreeIndex::open(const string& indexname, char mode)
{
	...
	{
		pf.read(0,treelist); // first read in how the index is organized
	...
	{	// array index 0 is never used since page 0 is used for the treelist
		...
		rootPid = StartPid;
		pf.write(0,treelist);
		treelist[StartPid].nnode->write(StartPid,pf);
		treelist[StartPid+1].lnode->write(StartPid+1,pf);
		treelist[StartPid+2].lnode->write(StartPid+2,pf);
...
RC BTreeIndex::close()
{
	...
	pf.write(0,treelist); // first write out how the index is organized
