...
#define N_NONLEAF_PAIRS ((PAGE_SIZE-sizeof(int))/(sizeof(int)+sizeof(PageId)))
#define N_LEAF_PAIRS    ((PAGE_SIZE-sizeof(int)-sizeof(PageId))/(sizeof(int)+sizeof(RecordId)))

#define BTREE_LEAF          0xA0000000
#define BTREE_NONL          0x80000000
#define BTREE_ROOT          0x40000000

#define COUNT_MASK	0x7FF 
#define FLAG_MASK	0xFFFF8000 
#define KV_COUNT	(data.fc & COUNT_MASK) 
#define BT_FLAGS	(data.fc & FLAG_MASK)

#define NONLEAF_VAL_COUNT (KV_COUNT*2 + 1)
...
class BTLeafNode {
 private:
  struct node_data{
    int fc;
    leaf_value kv_pairs[N_LEAF_PAIRS];
    PageId next;
    int padding[2];
  } data;
 public:
...
class BTNonLeafNode {
 private:
  struct node_data{
    int fc;
    int kv_pairs[N_NONLEAF_PAIRS*2+1];
  } data;
 public:
   /**
