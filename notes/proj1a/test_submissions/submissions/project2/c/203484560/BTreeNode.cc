...
{ 
	...
	{	
		...
		memcpy(pids, cursor, MAX_PID*sizeof(int));
		cursor += MAX_PID;
		
		//Get  keys[ ] from buffer
		memcpy(keys, cursor, MAX_KEYS*sizeof(int));
		cursor += MAX_KEYS;
	}
	else
...
{ 
	...
	//Copy pids[ ] to buffer
	memcpy(cursor, pids, MAX_PID*sizeof(int));
	cursor += MAX_PID;
	
	//Copy keys[ ] to buffer
	memcpy(cursor, keys, MAX_KEYS*sizeof(int));
	cursor += MAX_KEYS;
