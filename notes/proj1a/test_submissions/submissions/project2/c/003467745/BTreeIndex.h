...
class BTreeIndex {
 public:
  ...
  RC readForward(IndexCursor& cursor, int& key, RecordId& rid);
	...
	/// Gets the Initial Cursor in order to do a traversal
	RC getInitialCursor(IndexCursor& cursor);

	int getTreeHeight();
	int getRootPid();

	void printTree(PageId currentPid, int height);
	void printLeafNode(PageId currentPid);
	int count;
