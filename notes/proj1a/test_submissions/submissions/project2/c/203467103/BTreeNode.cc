...
// ||
// || rid | key | rid | key | ... | padding | TYPE | PARENT | NUMKEYS | NEXTNODE ||
// ||
// added: parent
...
// ||
// || key | pid | key | pid | ... | padding | TYPE | PARENT | NUMKEYS | PREVNODE ||
// ||
//
...
RC BTLeafNode::read(PageId pid, const PageFile& pf)
{
	...
	memcpy(&m_type, buffer+TYPE_LOC, sizeof(int));
	memcpy(&m_parent, buffer+PARENT_LOC, sizeof(int));
	memcpy(&m_next, buffer+NEXTNODE_LOC, sizeof(int));
	return rc;
}
...
RC BTLeafNode::write(PageId pid, PageFile& pf)
{
	RC rc;
	// check for invalid page id
	memcpy(buffer+TYPE_LOC, &m_type, sizeof(int));
	memcpy(buffer+NEXTNODE_LOC, &m_next, sizeof(int));
...
RC BTLeafNode::insert(int key, const RecordId& rid)
{
	...
	int dataSize = MAX_ENTRIES*ENTRY_SIZE - eid*ENTRY_SIZE - ENTRY_SIZE;
                              ...
                              BTLeafNode& sibling, int& siblingKey)
{
	...
	// ensure the type and parent is the same
	memcpy(buffer+TYPE_LOC, &m_type, sizeof(int));
	memcpy(buffer+PARENT_LOC, &m_parent, sizeof(int));
	memcpy(sibling.buffer+TYPE_LOC, &m_type, sizeof(int));
	memcpy(sibling.buffer+PARENT_LOC, &m_parent, sizeof(int));
...
int BTNonLeafNode::getType()
{
	return m_type;
}

RC BTNonLeafNode::setPrevNodePtr(PageId pid)
{
	memcpy(buffer+PREVNODE_LOC, &pid, sizeof(int));
	return 0;
}
