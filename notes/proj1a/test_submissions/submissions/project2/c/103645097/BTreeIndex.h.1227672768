...
// private:
  ...
  // stuff that needs to reside on-disk
  BTIndexHeader header;

  RC read();
  RC write();

  RC recursiveLocate(int searchKey, BTNonLeafNode *parent, int curDepth, IndexCursor &cursor);
  RC recursiveInsert(int key, const RecordId &id, BTNonLeafNode *parent, int curDepth, int &newKey, PageId &newPID);

  void initRoot(int key, const RecordId &id);
  bool empty() { return header.height == 0; }
};
