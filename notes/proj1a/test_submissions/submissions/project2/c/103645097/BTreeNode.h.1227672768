...
struct BTLeafNodeHeader
{
	int    nKeys;
	PageId nextSibling;

	BTLeafNodeHeader() : nKeys(0), nextSibling(-1) {}
};

struct BTLeafNodeData
{
	RecordId recordID;
	int      key;
};
...
class BTLeafNode {
public:
	// This should evaluate to 84
	static const int MAX_KEYS = 3;//(PageFile::PAGE_SIZE - sizeof(BTLeafNodeHeader)) / sizeof(BTLeafNodeData);

	// Disk-resident data
	BTLeafNodeHeader header;
	BTLeafNodeData   data[MAX_KEYS];
  public:
   /**
...
struct BTNonLeafNodeHeader
{
	int    nKeys;
	PageId firstChild;
	BTNonLeafNodeHeader() : nKeys(0), firstChild(-1) {}
};

struct BTNonLeafNodeData
{
	int    key;
	PageId pageID;
};
...
public:
	...
	static const int MAX_KEYS = 3;//(PageFile::PAGE_SIZE - sizeof(BTNonLeafNodeHeader)) / sizeof(BTNonLeafNodeData);

	BTNonLeafNodeHeader header;
	BTNonLeafNodeData   data[MAX_KEYS];
  public:
   /**
