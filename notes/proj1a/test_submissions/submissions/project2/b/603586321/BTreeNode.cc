...
RC BTLeafNode::insertAndSplit(int key, const RecordId& rid, BTLeafNode& sibling, int& siblingKey) { 
	...
	sibling.sibling = lns.sibling;

	// Set the current node to point to the new sibling
	lns.sibling = sibling.pid;
	...
	int num_keys_node = ceil((MAX_LEAF_KEYS + 1) / 2);
	...
	for (int i = 0; i < num_keys_sibling; i++) {
		sibling.keys[i] = temp_keys[num_keys_node+i];
		sibling.rids[i].pid = temp_rids[num_keys_node+i].pid;
		sibling.rids[i].sid = temp_rids[num_keys_node+i].sid;
	}
	sibling.key_count = num_keys_sibling;
...
RC BTNonLeafNode::insertAndSplit(int key, PageId pid, BTNonLeafNode& sibling, int& midKey) { 
	...
	// Transfer first half of the key, rid pairs to node
	int mid_keys_node = ceil((MAX_NONLEAF_KEYS + 1) / 2);
	for (int i = 0; i < mid_keys_node - 1; i++) {
		nlns.keys[i] = temp_keys[i];
		nlns.pids[i] = temp_pids[i];
	}
	nlns.pids[mid_keys_node] = temp_pids[mid_keys_node];
	nlns.key_count = mid_keys_node - 1;

	// Transfer second half of the key, rid pairs to sibling
	int num_keys_sibling = (MAX_NONLEAF_KEYS + 1) - mid_keys_node;
	for (int i = 0; i < num_keys_sibling; i++) {
		sibling.keys[i] = temp_keys[mid_keys_node+i];
		sibling.pids[i] = temp_pids[mid_keys_node+i];
	}
	sibling.pids[num_keys_sibling] = temp_pids[mid_keys_node];
	sibling.key_count = num_keys_sibling;
	
	// Return the first key of the sibling
	midKey = temp_keys[mid_keys_node];
