...
#define LENTRY_SIZE 12
#define NENTRY_SIZE 8
#define LMAXENTRY (PageFile::PAGE_SIZE - HEADER_SIZE) / LENTRY_SIZE
#define NMAXENTRY (PageFile::PAGE_SIZE - HEADER_SIZE) / NENTRY_SIZE
...
class BTLeafNode {
  public:
    BTLeafNode();
    RC clearBuffer();
    RC construct(char* inPtr, int numEntry);
    ...
    */
    RC insertAndSplit(int key, const RecordId& rid, BTLeafNode& sibling, const PageId& siblingPid, int& siblingKey);
