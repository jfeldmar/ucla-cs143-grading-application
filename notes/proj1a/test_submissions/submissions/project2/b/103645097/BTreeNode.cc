...
RC BTLeafNode::read(PageId pid, const PageFile& pf)
{
	...
	buffer += sizeof(BTLeafNodeHeader);

	// read in actual nodes
	memcpy(data, buffer, header.nKeys * sizeof(BTLeafNodeData));
...
RC BTLeafNode::write(PageId pid, PageFile& pf)
{
	...
	memcpy(buffer + sizeof(BTLeafNodeHeader), data, header.nKeys);
...
RC BTLeafNode::insert(int key, const RecordId& rid)
{
	...
	for(int j = i + 1; j <= header.nKeys; j++)
	{
...
RC BTNonLeafNode::read(PageId pid, const PageFile& pf)
{
	...
	buffer += sizeof(BTNonLeafNodeHeader);

	// read in actual nodes
	memcpy(data, buffer, header.nKeys * sizeof(BTNonLeafNodeData));
...
RC BTNonLeafNode::write(PageId pid, PageFile& pf)
{
	...
	memcpy(buffer + sizeof(BTNonLeafNodeHeader), data, header.nKeys);
...
RC BTNonLeafNode::insertAndSplit(int key, PageId pid, BTNonLeafNode& sibling, int& midKey)
{
	...
	midKey = data[MAX_KEYS / 2].key; // only need key value because BTree will handle the pointers
