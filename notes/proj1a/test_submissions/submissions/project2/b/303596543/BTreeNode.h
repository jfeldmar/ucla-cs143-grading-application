 ...
 */
	  ...
	  */
	typedef struct {
	// element key of tuple
	int			key;
	// Contains:
	// pid - index pageid
	// sid - entry inside node array
	RecordId	rid;
	} NodeEntry;
	static const int nNodes=(PageFile::PAGE_SIZE/sizeof(NodeEntry))-1;
	typedef struct {
	// leaf nodes can point to their neighbor but nonleaf nodes are null
	PageId	nextNode;
	// page id of this index node
	PageId	nodepid;
	// number of entries in this node
	int		count;
	// flag if this is a root node
	int		theroot;
	NodeEntry	node[nNodes];
	} NodeType;
...
class BTLeafNode {
  public:
    ...
    */
    int getKeyCount();
	void readNextEntry(int& eid,PageId& pid);
   /**
