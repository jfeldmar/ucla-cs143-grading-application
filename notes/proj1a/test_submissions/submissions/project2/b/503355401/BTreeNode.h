...
class BTLeafNode {
  public:
          ...
          /* Print out a minimap of node residency */
          void print();
...
private:
        ...
        static const int MAX_PAIRS = (PageFile::PAGE_SIZE - sizeof(Next)) / sizeof(Pair);

        /* Retrieves a handle to a Pair */
        int getPair(int eid, Pair &pair);
        int setPair(int eid, const Pair &pair);
        /* Retrieves a handle to the next pointer */
        int getNext(Next &next);
        int setNext(const Next &next);
        /* Binary search for key */
        int binarySearch(int searchKey, int begin, int end, int &eid);

        /* In-memory scratch node storage. Use read and write to save to disk. */
        char m_buffer[PageFile::PAGE_SIZE];
        int m_nkeys;                    /* number of resident keys */
}; 
...
private:
        ...
        static const int MAX_PAIRS = (PageFile::PAGE_SIZE - sizeof(Prev)) / sizeof(Pair);

        /* Retrieves a handle to a Pair */
        int getPair(int eid, Pair &pair);
        int setPair(int eid, const Pair &pair);
        /* Retrieves a handle to the Prev pointer */
        int getPrev(Prev &prev);
        int setPrev(const Prev &prev);
        /* Binary search for key */
        int binarySearch(int searchKey, int begin, int end, int &eid);
