...
RC BTLeafNode::insert(int key, const RecordId& rid)
{
	...
	//340 is close to 1024/3 as a LeafPair struct has 3 integers
	//We also need 3 ints for the first entry, so 3 + 343*3 = 1023
	if (count == 340)	{
		return RC_NODE_FULL;
	}

	//Expand the buffer if necessary, let's say by 20
	if (size == count)	{
		size += 20;
		LeafPair* copy = new LeafPair[size + 1];
...
RC BTNonLeafNode::insert(int key, PageId pid)
{
	...
	//Expand the buffer if necessary, let's say by 32
	if (size == count)	{
		size += 32;
		NonLeafPair* copy = new NonLeafPair[size + 2];
		for (int i = 0; i < count; ++i)	{
			copy[i] = buffer[i];
		}
		delete [] buffer;
...
{
	//If less than all the keys
	if (buffer[1].pid < searchKey)	{
		pid = first_pid;
		return 0;
	}

	//If greater or equal to one of the keys
	for (int i = 0; i < count; ++i)	{
		if (buffer[i+2].key >= searchKey)	{
			pid = buffer[i+2].pid;
			return 0;
		}
	}
