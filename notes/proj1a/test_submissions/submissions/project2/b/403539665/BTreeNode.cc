...
struct Entry{
	...
	Entry():rid(rid)
	{
		eid = 0;
		key = 0;
		privatebuffer = (char*) malloc (1024);
	}
...
{ 
	...
	memcpy(entry[0].privatebuffer,buffer,1024);
...
{ 
	if(pf.write(pid, entry[0].privatebuffer) == 0)
		return 0;
	else
		return -1;
}
...
{ 
	...
	if(currenteid < (nValue-1))
	{
		if(key < newkey)
		{
			...
			entry[currenteid].add(lookupeid,key,rid);
			currenteid++;
			return 0;
		}
	}
	...
	{
		entry[currenteid].add((lookupeid+1),key,rid);
		currenteid++;
		for(int i = 0; i < currenteid; i++)
		{
...
{ 
	...
	memcpy(nonentry[0].privatebuffer,buffer,1024);
	if(pf.read(pid,buffer) == 0)
		return 0;
	else
		return -1; 
...
{ 
	if(pf.write(pid, nonentry[0].privatebuffer) == 0)
		return 0;
	else
		return -1;
}
