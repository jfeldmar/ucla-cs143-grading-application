...
static const int N_LEAF = 85; //Leaf nodes
static const int N_NONLEAF = 127; //Nonleaf nodes
...
class BTLeafNode {
  public:
    ...
    */
	...
	// Helper Functions
	PageId getRecordPid(int index);

	int getRecordSid(int index);

	int getKey(int index);

	RC setRecordPid(int index, PageId pid);

	RC setRecordSid(int index, int sid);

	int setKey(int index, int key);
	
 // private:
	//BTLeafEntry leafArray[N_LEAF]; //Index of each member of the array irepresents eid
	char leafArray[PageFile::PAGE_SIZE]; //Index of each member of the array irepresents eid
}; 
...
class BTNonLeafNode {
  public:
   ...
   /**
    * Initialize a newly created BTNonLeafNode.
    * This constructor writes values of -1 to the pid sections
    * of the class buffer.
    */  
   ...
   */
	RC setPid(int index, PageId pid);
   ...
   * Return an error code if there is an error, 0 if succesful.
   */
	RC setKey(int index, int key);
	
  //private:
	//BTNonLeafEntry nonleafArray[N_NONLEAF]; //Index of each member of the array irepresents eid
	char nonLeafArray[PageFile::PAGE_SIZE];
}; 
