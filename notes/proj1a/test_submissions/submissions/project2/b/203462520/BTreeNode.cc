...
RC BTNode::locate(int searchKey, int& eid)
{
	...
	//fprintf(stdout, "Did not locate an entry!\n");
...
BTLeafNode::BTLeafNode()
{
				...
				numnodes = CONFIG_NODES;
				lastnode = numnodes - 1;
				memset(&nodepage, '\0', numnodes * sizeof(nodeEntry));
				setKeyCount(0);
				setLeafFlag();
}
...
RC BTLeafNode::insert(int key, const RecordId& rid)
{
				...
				keycount = getKeyCount();
				int eid;
				//fprintf(stdout, "In LN::insert with key %d.\n", key);
				...
				if (eid < keycount) { // if eid is going to overwrite
								...
								//fprintf(stdout, "Shifting entries...\n");
				...
				//fprintf(stdout, "Insertion complete:\n");
				print();
...
{ 
				...
				//fprintf(stdout, "Inserting and Splitting (leaf)\n");
				...
				//fprintf(stdout, "Splitting entries...\n");
...
RC BTNonLeafNode::insert(int key, PageId pid)
{
		...
		//fprintf(stdout, "Inserting %d into NL node...\n", pid);
		print();
	
		eid = 7;
		if ((rc = locate(key, eid)) < 0) {
						if (rc == RC_NO_SUCH_RECORD) {
										//fprintf(stdout, "No match, eid set to %d\n", keycount);
										eid = keycount;
						} else {
							return rc; // error
						}
...
{ 
				 ...
				 * erase copied nodeEntries from orig
				 * */
				int split = (numnodes - 2) / 2;
				memcpy(&sibling.nodepage[0], &nodepage[split],
							 sizeof(nodeEntry)*(numnodes - split));
				memset(&nodepage[split], '\0', sizeof(nodeEntry)*(numnodes - split));
				 ...
				 * on the new sibling
				 * */
				int last = numnodes - split - 1; // last copied record
				sibling.nodepage[lastnode].rid.pid 
								= sibling.nodepage[last].rid.pid;
				sibling.nodepage[last].rid.pid = 0;

				/* we need to update the keycount in the new sibling
				 * and in the original node and erase the leftovers
				 * */
				sibling.setKeyCount(sibling.nodepage[last].key);
				sibling.nodepage[last].key = 0;
