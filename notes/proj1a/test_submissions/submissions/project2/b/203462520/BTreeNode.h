...
#define CONFIG_NODES 6

	public:

				int myParent; // member to hold curent parent
				int mypid; // member to hold current page id?
				int numnodes; // number of nodeEntries in nodepage
				int lastnode; // numnodes - 1 -- the last node
				//nodeEntry nodepage[PageFile::PAGE_SIZE / sizeof(nodeEntry)]; // buffer for page
				nodeEntry nodepage[CONFIG_NODES]; // buffer for page
