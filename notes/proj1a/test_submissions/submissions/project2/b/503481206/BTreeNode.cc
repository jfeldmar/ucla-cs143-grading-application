...
//#endif
  ...
  do { // do-while b/c need to create at least the pair of dummies ONLY
    if (is_old_node && buffer[i] == 0) // if node is new
      break;
    RecordId record_id;
...
{ 
	...
	//set the current next node ptr to the sibling and set siblings next node ptr to whatever this was pointing at
...
{ //XXX: note that this is untested
  ...
  do { // do-while b/c need to create at least one dummy key
    PageId pid = buffer[i] == 0 ? -1 : buffer[i];
    // do not run over end of buffer
    int key = i+1 >= limit || buffer[i+1] == 0 ? INT_MIN : buffer[i+1];
    node.insert(pair<int, PageId>(key, pid));
    //XXX: Assume RecordId cannot be 0. Assume Key cannot be 0.
    i += 2;
  } while (buffer[i] != 0 && i < limit);
