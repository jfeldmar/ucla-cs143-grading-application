...
RC BTLeafNode::insert(int key, const RecordId& rid)
{
  ...
  {
	dtob(getPos(0), key);
	dtob(getPos(0)+8, rid.pid);
	dtob(getPos(0)+16, rid.sid);
  }
  else
  {
    int eid = 0;
    ...
    shift(eid);
	dtob(getPos(eid), key);
	dtob(getPos(eid)+8, rid.pid);
	dtob(getPos(eid)+16, rid.sid);	
  }
...
RC BTLeafNode::readEntry(int eid, int& key, RecordId& rid)
{
  ...
  int p = getPos(eid);
  key = getKey(eid);
  rid.pid = btod(getPos(eid)+8);
  rid.sid = btod(getPos(eid)+16);
  return 0;
}
...
void BTLeafNode::dtob(int pos, int n)
{
  int div = 128;
...
RC BTNonLeafNode::insertAndSplit(int key, PageId pid, BTNonLeafNode& sibling, int& midKey)
{
  ...
  sibling.readEntry(0, midKey, tpid);
...
{
  return (nkeys == TOTAL_NONLEAF_KEYS) ? true : false;
}
...
// Convert eights bits into decimal starting at position pos
// and return it
int BTNonLeafNode::btod(int pos)
{
  int div = 128;
  int max = pos+8;
  int total = 0;
...
// Convert decimal to binary and put it in the eight bits
// starting at position pos
void BTNonLeafNode::dtob(int pos, int n)
{
  int div = 128;
  for (int i = pos; i < pos + 8; i++)
  {
