...
class BTLeafNode {
	...
	* @return 0 if successful. Return an error code if there is an error.
	*/
	RC setEntry(int eid, int key, RecordId rid);
 ...
 * N = 128: 128 pointers and 127 keys per node
 */
class BTNonLeafNode {
  public:
   /**
    ...
    */
	...
	/**
	* Set the (key, pid) pair at the eid entry.
	* @param eid[IN] the entry number to set the (key, rid) pair as
	* @param key[OUT] the key for the entry
	* @param pid[OUT] the PageId for the entry
	* @return 0 if successful. Return an error code if there is an error.
	*/
	RC setEntry(int eid, int key, PageId pid);
