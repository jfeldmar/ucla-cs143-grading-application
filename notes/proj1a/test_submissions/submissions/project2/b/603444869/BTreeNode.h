...
class BTLeafNode {
  public:
    ...
    */
    RC insert(int a_key, const RecordId& rid);
    ...
    */
    RC insertAndSplit(int a_key, const RecordId& rid, BTLeafNode& sibling, int& siblingKey);
    ...
    * @return 0 if successful. Return an error code if there is an error.
    */
    RC readEntry(int eid, int& a_key, RecordId& rid);
...
class BTNonLeafNode {
  public:
        int numKeys;
        int key[MAX_RECORDS];
        PageId pageid[MAX_RECORDS];
        PageId first_pid;

    BTNonLeafNode();
    //constructor: same as initializeRoot
    BTNonLeafNode(PageId pid1, int key, PageId pid2);
